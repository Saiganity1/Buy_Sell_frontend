...
export default function EditListingScreen({ route, navigation }) {
  ...
  const load = async () => {
    ...
    const vs = Array.isArray(data.variants) ? data.variants.map((v) => ({ id: v.id, name: v.name, stock: String(v.stock) })) : [];
    setVariants(vs);
  };
  ...
  const save = async () => {
    try {
      setLoading(true);
      const payload = { title, description, price, has_variants: hasVariants };
      if (hasVariants) {
