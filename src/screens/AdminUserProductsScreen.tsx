...
interface Product { id: number; title: string; price: string; created_at?: string; image?: string; image_url?: string }

export default function AdminUserProductsScreen({ route, navigation }: any) {
  const { userId, username } = route.params;
  const [products, setProducts] = useState<Product[]>([]);

  const load = async () => {
    // Use public products endpoint filtered by seller to avoid admin-only access issues
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      />
      <Snack visible={!!snack} message={snack?.msg || ''} type={snack?.type || 'success'} onHide={() => setSnack(null)} />
    </View>
  );
}
