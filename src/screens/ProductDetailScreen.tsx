...
import { View, Text, StyleSheet, Image, Alert, ScrollView, Modal, TextInput } from 'react-native';
...
export default function ProductDetailScreen({ route, navigation }: any) {
  ...
  const productOutOfStock = useMemo(() => {
    if (product?.has_variants) return false; // handled per-variant
  ...
  const addToCart = async (quantity: number) => {
    try {
      setLoading(true);
      const payload: any = { product_id: product.id, quantity };
  ...
  const handleAddPress = () => {
    // Ensure variant selection first if needed
    if (product?.has_variants) {
      if (!selectedVariantId) { Alert.alert('Please select a variant'); return; }
      if (selectedVariant && Number(selectedVariant.stock) <= 0) { Alert.alert('Out of stock', 'Selected variant is out of stock.'); return; }
